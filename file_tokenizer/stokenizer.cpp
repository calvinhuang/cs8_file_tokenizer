...
void STokenizer::make_table(int _table[][MAX_COLUMNS])
{
	...
	set_table_row(_table, ALPHA_START + 1, (int) 'A', (int) 'Z', ALPHA_START + 1);
	set_table_row(_table, ALPHA_START + 1, (int) '0', (int) '9', ALPHA_START + 1); // R2D2, C3PO, RX9, etc.
...
bool STokenizer::get_token(int start_state, string& token)
{
	...
	while (start_state > -1 && !done())
	{
		...
		{
			// not end-of-string so check next state
	...
	{
//		std::cout << "found token: " << token << " ending at " << _pos << "\n";
